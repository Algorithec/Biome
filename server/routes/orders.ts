...
router.post("/:orderId/payment-intent", authRequired(), async (req: Request, res: Response) => {
  const orderId = req.params.orderId;
  const order = await orderRepo.getById(orderId);
  if (!order || order.userId !== req.ctx!.userId!) {
    res.status(404).json({ error: "ORDER_NOT_FOUND" });
  ...
  const intentId = typeof json?.intent?.intentId === "string" ? json.intent.intentId : null;
  const cfOrderId = typeof json?.intent?.orderId === "string" ? json.intent.orderId : null;
  const paymentSessionId = typeof json?.intent?.paymentSessionId === "string" ? json.intent.paymentSessionId : null;
  ...
  const updated = await orderRepo.updateById(order.id, { status: "PAYMENT_PENDING", paymentIntentId: intentId, metadata });
