...
export function startOrderPaymentsSync() {
  const enabled =
    (process.env.ORDER_PAYMENTS_SYNC_ENABLED || "true").toLowerCase() !==
    "false";
  if (!enabled) return;

  const intervalMs = Math.max(
    10_000,
    Number(process.env.ORDER_PAYMENTS_SYNC_INTERVAL_MS || 30_000)
  );
  const limit = Math.max(
    1,
    Math.min(200, Number(process.env.ORDER_PAYMENTS_SYNC_LIMIT || 50))
  );
  ...
  const tick = async () => {
    ...
    await Promise.all(
      pending.map(async order => {
        ...
        try {
          const r = await fetch(
            `${paymentsBaseUrl()}/v1/payment_intents/${encodeURIComponent(order.paymentIntentId)}`
          );
          if (!r.ok) return;
