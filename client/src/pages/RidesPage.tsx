...
export default function RidesPage() {
  ...
  const filteredQuotes = useMemo(
    () => quotes.filter(q => q.type === activeType),
    [quotes, activeType]
  );
  ...
  const fastestId = useMemo(() => {
    ...
    for (const q of filteredQuotes) {
      if (!best || q.etaMinutes < best.eta)
        best = { id: q.id, eta: q.etaMinutes };
    }
    return best?.id ?? null;
  }, [filteredQuotes]);
  ...
  const renderRoute = async (
    ...
    try {
      ...
      if (Array.isArray(coords) && coords.length > 1) {
        latLngs = coords
          ...
          .filter(
            (x: any): x is [number, number] =>
              Array.isArray(x) && Number.isFinite(x[0]) && Number.isFinite(x[1])
          );
      }
    } catch {
      latLngs = [
    ...
    const allowAutoFit =
      opts?.forceFit === true ||
      Date.now() - lastUserMapActionAtRef.current >= 900;
    if (allowAutoFit)
      mapRef.current.fitBounds(poly.getBounds(), { padding: [42, 42] });
  };
  ...
  const applyPoint = async (
    ...
    fitToMarkers(
      which === "pickup" ? point : pickupRef.current,
      which === "dropoff" ? point : dropoffRef.current
    );
    ...
    try {
      ...
      if (which === "pickup")
        setPickupText(`${point.lat.toFixed(5)}, ${point.lng.toFixed(5)}`);
      else setDropoffText(`${point.lat.toFixed(5)}, ${point.lng.toFixed(5)}`);
    }
  };

  const geocodeToPoint = async (
    q: string
  ...
  const geocodeToPointWithLabel = async (
    q: string
  ): Promise<{
    point: { lat: number; lng: number };
    label?: string;
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!result) return;
        await applyPoint("pickup", result.point, {
          label: result.label,
          doReverse: false,
        });
      } catch {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(async () => {
      ...
      try {
        const result = await geocodeToPointWithLabel(q);
        if (!result) return;
        await applyPoint("dropoff", result.point, {
          label: result.label,
          doReverse: false,
        });
      } catch {
          ...
          >
                      ...
                      onClick={() => {
                        ...
                        try {
                          ...
                          void applyPoint(
                            ...
                            { label: parsed.label, doReverse: false }
                          );
                          if (parsed.label) {
                            if (activeField === "pickup")
                              setPickupText(parsed.label);
                  ...
                  onMapReady={(map: L.Map) => {
                    ...
                    map.on("click", async (e: L.LeafletMouseEvent) => {
                      ...
                      if (activeFieldRef.current === "pickup") {
                        await applyPoint("pickup", point, {
                          doReverse: true,
                          forcePan: true,
                        });
                        setActiveField("dropoff");
                      } else {
                        await applyPoint("dropoff", point, {
                          doReverse: true,
                          forcePan: true,
                        });
                        setSelectMode(false);
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                                ...
                                {ride.surgeMultiplier ? (
                                  <span>Surge x{ride.surgeMultiplier}</span>
                                ) : null}
                              </div>
                            </div>
                          </div>
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await ordersAPI.createOrder({
                            ...
                            amount: {
                              currency: "INR",
                              amount: Math.max(1, Math.round(selected.fare)),
                            },
                            metadata: {
                              quoteId: selected.id,
                              pickup,
                          ...
                          toast.message("Opening provider checkout.", {
            ...
            <nav className="app-bottom-nav" aria-label="Bottom navigation">
              <div className="app-bottom-nav-inner">
                ...
                <button
                  className="app-bottom-nav-item app-bottom-nav-item-active"
                  type="button"
                  aria-label="Rides"
                >
                  <span className="app-bottom-nav-bubble">
